...
import 'logging.dart';
import 'package:json_parse_model/src/source_config.dart';
...
class Json2ModelPrintBuilder implements Builder {
  ...
  Json2ModelPrintBuilder({required this.options});
  ...
  FutureOr<void> build(BuildStep buildStep) async {
    try {
      ...
      jlogger.info('contents=$contents');
      final sourceConfig = SourceParse.parse(contents);

      Map<String, dynamic>? jsonMap = await getSourceJsonObject(sourceConfig);
      if (jsonMap == null || jsonMap.isEmpty) {
        jlogger.error(
            '$tag jsonMap is null, please check your assets/json/xxx.json file exist or formatter!');
        return;
      }
      final data = jsonMap['data'];
      if (data is bool || data is String || data is int) {
        jlogger.error('$tag is bool or string or int');
        return;
      }
      Map<String, dynamic> source = {};
      if (data is List) {
        if (data.isEmpty) {
          jlogger.error('data is List but isEmpty');
          return;
        }
      ...
      _buildClassCode(
        ...
        copyWith: sourceConfig.copyWith,
      );
  ...
  Future<Map<String, dynamic>?> getSourceJsonObject(
      JsonSourceConfig sourceConfig) async {
...
void _buildClassCode({
  ...
  /// copyWith
  if (copyWith) {
    final copyWithMetod = _buildCopyWithMethod(className, fields);
    cb.methods.add(copyWithMetod);
  }

  final model = cb.build();
...
Method _buildCopyWithMethod(String className, List<Field> fields) {
  final toJsonMethodBody = StringBuffer('');
  for (final e in fields) {
    toJsonMethodBody.writeln("${e.name}: ${e.name} ?? this.${e.name},");
  }
  final toJsonMethod = Method(
    (b) => b
      ..name = "copyWith"
        ...
        fields.map(
          (m) => Parameter(
            (b) => b
              ..name = m.name
              ..named = true
              ..type = refer('${m.type?.symbol}?').type,
          ),
        ),
      )
      ..returns = TypeReference((b) => b..symbol = className)
      ..body = Code('''return $className(${toJsonMethodBody.toString()});'''),
  );
  return toJsonMethod;
}
